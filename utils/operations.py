...
from utils.losses import CE_Mask
def _concat(xs):
...
def hessian_vector_product_ssl(W, W_k, img_ssl_q, img_ssl_k, vector, weights, queue,r=1e-2):
    R = r / _concat(vector).norm()

    for p, v in zip(W.parameters(), vector):
      p.data.add_(R, v)

    loss = ssl_step(W,W_k, img_ssl_q, img_ssl_k, queue=queue, A_ssl=weights, criterion=CE_Mask())
    grads_p = torch.autograd.grad(loss,weights)
